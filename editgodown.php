<?php 
	...
	if(isset($_POST['btnsav']))
	{
		...
		$a=mysql_query("select max(code) as code from godown");
		...
		include("dbconnect.php");
		$a=mysql_query("insert into godown(code, descrip) values('$xcode', '$xdescrip')");
		if($a==1)
		{
			?>
				<script>alert("Record Inserted Succssfully.....!"); location.replace("godown.php"); </script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Inserted.....!"); location.replace("godown.php"); </script>
			<?php
		}
	}
	else if(isset($_POST['btnupd']))
	{
		extract($_POST);
		include("dbconnect.php");
		$b=mysql_query("update godown set code='$xcode', descrip='$xdescrip' where code=".$_GET['code']);
		if($b==1)
		{
			?>
				<script>alert("Record Updated Succssfully.....!"); location.replace("godown.php");</script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Updated.....!"); location.replace("godown.php");</script>
			<?php
		}
	}
			...
			<form method="post" id="form">
				...
				<?php
					if(isset($_GET['code']))
					{
						extract($_GET);
						include("dbconnect.php");
						$e=mysql_query("select * from godown where code=".$_GET['code']);
