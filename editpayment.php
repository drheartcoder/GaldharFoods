<?php 
	...
	if(isset($_POST['btnsav']))
	{
		...
		$a=mysql_query("select max(id) as id from payment");
		...
		include("dbconnect.php");
		$a=mysql_query("insert into payment(id, payment, days) values('$xid', '$xpayment', '$xdays')");
		if($a==1)
		{
			?>
				<script>alert("Record Inserted Succssfully.....!"); location.replace("payment.php"); </script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Inserted.....!"); location.replace("payment.php"); </script>
			<?php
		}
	}
	else if(isset($_POST['btnupd']))
	{
		extract($_POST);
		include("dbconnect.php");
		$b=mysql_query("update payment set id='$xid', payment='$xpayment', days='$xdays' where id=".$_GET['id']);
		if($b==1)
		{
			?>
				<script>alert("Record Updated Succssfully.....!"); location.replace("payment.php");</script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Updated.....!"); location.replace("payment.php");</script>
			<?php
		}
	}
			...
			<form method="post" id="form">
				...
				<?php
					if(isset($_GET['id']))
					{
						extract($_GET);
						include("dbconnect.php");
						$e=mysql_query("select * from payment where id=".$_GET['id']);
				...
				<div style="margin-left:38%; margin-right:35%;">
					<span style="width:80%; font-size:17px;">
						...
						<span style="float:right; width:70%;">
							...
							<input type="text" name="xpayment" value="<?php echo $payment; ?>" /><br />
							<input type="text" name="xdays" value="<?php echo $days; ?>" /><br />
						</span>
					</span>
