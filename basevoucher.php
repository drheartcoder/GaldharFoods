...
$result=mysql_query("SELECT count(*) AS total FROM voumast");
...
include("dbconnect.php");
$result = mysql_query("SELECT * FROM voumast ORDER BY srno Asc LIMIT $from , $perPage");
?>
<!--Pageing Ends-->
...
<html>
	<head>
		...
		function searchelement(searchFld)
		{
			...
			{	
				...
				var fn = "basevoucher.php?mt_head="+xt1;
				x.open("GET",fn);
	...
	<body onLoad="searchelement('mt_head')">
		<div style="height:745px;">
			...
			<form method="post">
			<br>
				<a href="editbasevoucher.php" style="margin-left:10%; font-size:18px;">New Base Voucher Setup</a>
				...
				<table border="1" cellspacing="0" cellpadding="5" width="100%" align="center">
					<thead>
						<tr bgcolor="grey" align="center" style="color:white;">
							...
							<th width="35%;">Voucher Type</th>
							<th width="35%;">Voucher Name</th>
							...
							while($row = mysql_fetch_array($result))
							{
								...
								<tbody>
									<tr align="center" class="<?php echo $class?>">
										<td width="10%;"><?=$srno?></td>
										<td width="35%;"><?=$vou_type?></td>
										<td width="35%;"><?=$vou_name?></td>
										<td width="12%;">
											<a href="editbasevoucher.php?srno=<?=$srno?>" class="button1" style="text-decoration:none">Edit</a>&nbsp;&nbsp;
											<a href="delbasevoucher.php?srno=<?=$srno?>" class="button1" style="text-decoration:none" onClick="javascript: return confirm('Are you SURE you wish to Delete?');">Delete</a>
										</td>
									</tr>
								</tbody>
							<?php
