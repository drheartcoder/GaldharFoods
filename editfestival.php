<?php 
	...
	if(isset($_POST['btnsav']))
	{
		...
		$a=mysql_query("select max(id) as id from festival");
		while($db1=mysql_fetch_array($a))
		{
			extract($db1);
			$yid = $db1['id'];
		}
        $xid = str_pad($yid+1,3,"0",STR_PAD_LEFT);   
		
		include("dbconnect.php");
		$a=mysql_query("insert into festival(id, descrip, active) values('$xid', '$xdescrip', '$xactive')");
		if($a==1)
		{
			?>
				<script>alert("Record Inserted Succssfully.....!"); location.replace("festival.php"); </script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Inserted.....!"); location.replace("festival.php"); </script>
			<?php
		}
	}
	else if(isset($_POST['btnupd']))
	{
		extract($_POST);
		include("dbconnect.php");
		$b=mysql_query("update festival set id='$xid', descrip='$xdescrip', active='$xactive' where id=".$_GET['id']);
		if($b==1)
		{
			?>
				<script>alert("Record Updated Succssfully.....!"); location.replace("festival.php");</script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Updated.....!"); location.replace("festival.php");</script>
			<?php
		}
	}
			...
			<form method="post" id="form">
				...
				<?php
					if(isset($_GET['id']))
					{
						extract($_GET);
						include("dbconnect.php");
						$e=mysql_query("select * from festival where id=".$_GET['id']);
				...
				<div style="margin-left:38%; margin-right:35%;">
					<span style="width:80%; font-size:17px;">
						...
						<span style="float:right; width:70%;">
							...
							<input type="text" name="xactive" value="<?php echo $active; ?>" /><br />
						</span>
					</span>
