<?php 
	...
	if(isset($_POST['btnsav']))
	{
		...
		$a=mysql_query("select max(id) as id from prodplan");
		...
		include("dbconnect.php");
		$a=mysql_query("insert into prodplan(id, descrip) values('$xid', '$xdescrip')");
		if($a==1)
		{
			?>
				<script>alert("Record Inserted Succssfully.....!"); location.replace("prodplan.php"); </script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Inserted.....!"); location.replace("prodplan.php"); </script>
			<?php
		}
	}
	else if(isset($_POST['btnupd']))
	{
		extract($_POST);
		include("dbconnect.php");
		$b=mysql_query("update prodplan set id='$xid', descrip='$xdescrip' where id=".$_GET['id']);
		if($b==1)
		{
			?>
				<script>alert("Record Updated Succssfully.....!"); location.replace("prodplan.php");</script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Updated.....!"); location.replace("prodplan.php");</script>
			<?php
		}
	}
			...
			<form method="post" id="form">
				...
				<?php
					if(isset($_GET['id']))
					{
						extract($_GET);
						include("dbconnect.php");
						$e=mysql_query("select * from prodplan where id=".$_GET['id']);
				...
				<div style="margin-left:38%; margin-right:35%;">
					<span style="width:80%; font-size:17px;">
						...
						<span style="float:right; width:70%;">
							<input type="text" readonly name="xid" value="<?php echo $id; ?>" style="cursor:default;" /><br />
