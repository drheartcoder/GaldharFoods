<?php 
	...
	if(isset($_POST['btnsav']))
	{
		...
		$a=mysql_query("select max(gp1) as code from itmgrp2");
		...
		include("dbconnect.php");
		$a=mysql_query("insert into itmgrp2(gp1, grpnm1) values('$xcode', '$xgrpnm1')");
		if($a==1)
		{
			?>
				<script>alert("Record Inserted Succssfully.....!"); location.replace("itemsubgroup2.php"); </script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Inserted.....!"); location.replace("itemsubgroup2.php"); </script>
			<?php
		}
	}
	else if(isset($_POST['btnupd']))
	{
		extract($_POST);
		include("dbconnect.php");
		$b=mysql_query("update itmgrp2 set gp1='$xgp1', grpnm1='$xgrpnm1' where gp1=".$_GET['gp1']);
		if($b==1)
		{
			?>
				<script>alert("Record Updated Succssfully.....!"); location.replace("itemsubgroup2.php");</script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Updated.....!"); location.replace("itemsubgroup2.php");</script>
			<?php
		}
	}
	...
	{
		header('location: itemsubgroup2.php');
	}
			...
			<form method="post" id="form">
				...
				<?php
					if(isset($_GET['gp1']))
					{
						extract($_GET);
						include("dbconnect.php");
						$e=mysql_query("select * from itmgrp2 where gp1=".$_GET['gp1']);
