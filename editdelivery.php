<?php 
	...
	if(isset($_POST['btnsav']))
	{
		...
		$a=mysql_query("select max(id) as id from delivery");
		...
		include("dbconnect.php");
		$a=mysql_query("insert into delivery(id, descrip) values('$xid', '$xdescrip')");
		if($a==1)
		{
			?><script>alert("Record Inserted Succssfully.....!"); location.replace("delivery.php"); </script><?php
		}
		else
		{
			?><script>alert("Record Not Inserted.....!"); location.replace("delivery.php"); </script><?php
		}
	}
	else if(isset($_POST['btnupd']))
	{
		extract($_POST);
		include("dbconnect.php");
		$b=mysql_query("update delivery set id='$xid', descrip='$xdescrip' where id=".$_GET['id']);
		if($b==1)
		{
			?><script>alert("Record Updated Succssfully.....!"); location.replace("delivery.php");</script><?php
		}
		else
		{
			?><script>alert("Record Not Updated.....!"); location.replace("delivery.php");</script><?php
		}
	}
			...
			<form method="post" id="form">
				...
				<?php
					if(isset($_GET['id']))
					{
						extract($_GET);
						include("dbconnect.php");
						$e=mysql_query("select * from delivery where id=".$_GET['id']);
