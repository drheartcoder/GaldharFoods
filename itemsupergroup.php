...
$result=mysql_query("SELECT count(*) AS total FROM itmtype");
...
include("dbconnect.php");
$result = mysql_query("SELECT * FROM itmtype ORDER BY itmcd Asc LIMIT $from , $perPage");
?>
<!--Pageing Ends-->
...
<html>
	<head>
		...
		function searchelement(searchFld)
		{
			...
			{	
				...
				var fn = "itemsupergroup.php?mt_head="+xt1;
				x.open("GET",fn);
	...
	<body onLoad="searchelement('mt_head')">
		<div style="height:745px;">
			...
			<form method="post">
			<br>
				<a href="edititemsupergroup.php" style="margin-left:10%; font-size:18px;">New Item Super Group</a>
						...
						while($row = mysql_fetch_array($result))
						{
							...
							<tbody>
								<tr align="center" class="<?php echo $class?>">
									<td width="10%;"><?=$itmcd?></td>
									<td width="50%;"><?=$itmtype?></td>
									<td width="12%;">
										<a href="edititemsupergroup.php?itmcd=<?=$itmcd?>" class="button1" style="text-decoration:none">Edit</a>&nbsp;&nbsp;
										<a href="delitemsupergroup.php?itmcd=<?=$itmcd?>" class="button1" style="text-decoration:none" onClick="javascript: return confirm('Are you SURE you wish to Delete?');">Delete</a>
									</td>
								</tr>
							</tbody>
							<?
