...
$result=mysql_query("SELECT count(*) AS total FROM custtype");
...
include("dbconnect.php");
$result = mysql_query("SELECT * FROM custtype ORDER BY typecd Asc LIMIT $from , $perPage");
?>
<!--Pageing Ends-->
...
<html>
	<head>
		...
		function searchelement(searchFld)
		{
			...
			{	
				...
				var fn = "custtype.php?mt_head="+xt1;
				x.open("GET",fn);
	...
	<body onLoad="searchelement('mt_head')">
		<div style="height:745px;">
			...
			<form method="post">
			<br>
				<a href="editcusttype.php" style="margin-left:10%; font-size:18px;">New Customer Type</a>
						...
						while($row = mysql_fetch_array($result))
						{
							...
							<tbody>
								<tr align="center" class="<?php echo $class?>">
									<td width="10%;"><?=$typecd?></td>
									<td width="50%;"><?=$cust_type?></td>
									<td width="12%;">
										<a href="editcusttype.php?typecd=<?=$typecd?>" class="button1" style="text-decoration:none">Edit</a>&nbsp;&nbsp;
										<a href="delcusttype.php?typecd=<?=$typecd?>" class="button1" style="text-decoration:none" onClick="javascript: return confirm('Are you SURE you wish to Delete?');">Delete</a>
									</td>
								</tr>
							</tbody>
							<?
