...
$result=mysql_query("SELECT count(*) AS total FROM itmgrp");
...
include("dbconnect.php");
$result = mysql_query("SELECT * FROM itmgrp ORDER BY gp Asc LIMIT $from , $perPage");
?>
<!--Pageing Ends-->
...
<html>
	...
	<body onLoad="searchelement('mt_head')">
		<div style="height:745px;">
			...
			<form method="post">
			<br>
				<a href="edititemgroup.php" style="margin-left:10%; font-size:18px;">New Item Group</a>
						...
						while($row = mysql_fetch_array($result))
						{
							...
							<tbody>
								<tr align="center" class="<?php echo $class?>">
									<td width="10%;"><?=$gp?></td>
									<td width="50%;"><?=$grpnm?></td>
									<td width="12%;">
										<a href="edititemgroup.php?gp=<?=$gp?>" class="button1" style="text-decoration:none">Edit</a>&nbsp;&nbsp;
										<a href="delitemgroup.php?gp=<?=$gp?>" class="button1" style="text-decoration:none" onClick="javascript: return confirm('Are you SURE you wish to Delete?');">Delete</a>
									</td>
								</tr>
							</tbody>
							<?
