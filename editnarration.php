<?php 
	...
	if(isset($_POST['btnsav']))
	{
		...
		$a=mysql_query("select max(code) as code from naramast");
		...
		include("dbconnect.php");
		$a=mysql_query("insert into naramast(code, descrip) values('$xcode', '$xdescrip')");
		if($a==1)
		{
			?>
				<script>alert("Record Inserted Succssfully.....!"); location.replace("narration.php"); </script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Inserted.....!"); location.replace("narration.php"); </script>
			<?php
		}
	}
	else if(isset($_POST['btnupd']))
	{
		extract($_POST);
		include("dbconnect.php");
		$b=mysql_query("update naramast set code='$xcode', descrip='$xdescrip' where code=".$_GET['code']);
		if($b==1)
		{
			?>
				<script>alert("Record Updated Succssfully.....!"); location.replace("narration.php");</script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Updated.....!"); location.replace("narration.php");</script>
			<?php
		}
	}
			...
			<form method="post" id="form">
				...
				<?php
					if(isset($_GET['code']))
					{
						extract($_GET);
						include("dbconnect.php");
						$e=mysql_query("select * from naramast where code=".$_GET['code']);
