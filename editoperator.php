<?php 
	...
	if(isset($_POST['btnsav']))
	{
		...
		$a=mysql_query("select max(code) as code from operator");
		...
		include("dbconnect.php");
		$a=mysql_query("insert into operator(code, descrip, nickname) values('$xcode', '$xdescrip', '$xnickname')");
		if($a==1)
		{
			?>
				<script>alert("Record Inserted Succssfully.....!"); location.replace("operator.php"); </script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Inserted.....!"); location.replace("operator.php"); </script>
			<?php
		}
	}
	else if(isset($_POST['btnupd']))
	{
		extract($_POST);
		include("dbconnect.php");
		$b=mysql_query("update operator set code='$xcode', descrip='$xdescrip', nickname='$xnickname' where code=".$_GET['code']);
		if($b==1)
		{
			?>
				<script>alert("Record Updated Succssfully.....!"); location.replace("operator.php");</script>
			<?php
		}
		else
		{
			?>
				<script>alert("Record Not Updated.....!"); location.replace("operator.php");</script>
			<?php
		}
	}
			...
			<form method="post" id="form">
				...
				<?php
					if(isset($_GET['code']))
					{
						extract($_GET);
						include("dbconnect.php");
						$e=mysql_query("select * from operator where code=".$_GET['code']);
